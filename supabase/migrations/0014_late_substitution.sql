-- Aspatuck Tournaments — late substitution helper
--
-- After a draw is set, the TD frequently needs to slot a new player into an
-- existing bracket position (late entry replacing a withdrawal, an injury
-- replacement, etc.). The existing td_replace_participant requires a
-- participant id, which means the TD first has to manually add the person as
-- a participant. This RPC collapses that into one step from the club
-- directory.
...
begin
  ...
  -- Reuse the tournament participant if it already exists.
