-- 0065_td_pair_team_into_double_bye.sql
--
-- Extend td_pair_team_into_bye_slot (0063) to accept R1 matches with
-- BOTH sides null, not just the "single bye" case where exactly one
-- side is populated. This happens when a bye-winner is later withdrawn
-- (the _unwind_entry_from_matches helper in 0052 nulls both pointers
-- and the winner pointer, leaving the match status='pending' with no
-- contestants). The TD needs to drop a new team into one of those
-- empty slots without regenerating.
--
-- Single-bye fill (one side populated):
--   * Same behaviour as before — the new team goes into the empty
--     side, match becomes pending, the previous bye-winner's R2
--     auto-advance is unwound so they have to actually beat the new
--     team to advance.
--
-- Double-bye fill (both sides null):
--   * Match had nothing in either side and nothing to advance. The new
--     team takes side 'a' (arbitrary; either side is equivalent when
--     the other is empty), the match is marked confirmed with the new
--     team as the bye winner, and advance_winner propagates them into
--     R2 the same way generate_draw does for fresh byes.
--
-- The two cases share validation, locking, participant resolution and
-- duplicate checks. The split only affects the final match-write +
-- bracket-walk step.
...
begin
  ...
  -- Locate the R2 match the bye winner is/was auto-advanced into.
  ...
  -- For single-bye the R2 status check protects us from rewriting a
  -- round the bye winner already played. For double-bye there's no
  -- previous winner so R2 status is irrelevant.
  if not v_double_bye and v_next_match.id is not null
     and v_next_match.status <> 'pending' then
    raise exception
  ...
  if v_double_bye then
    -- The new team takes side 'a' and is itself the bye winner; the
    -- match goes confirmed and advance_winner propagates them into R2
    -- the same way generate_draw seeds bye advancement.
    update matches
    ...
    -- Standard single-bye fill: slot into the empty side, match
    -- becomes a real pending match, and the previous bye winner's
    -- auto-advance into R2 is unwound.
