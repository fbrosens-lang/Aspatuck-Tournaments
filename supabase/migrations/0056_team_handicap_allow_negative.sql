-- Aspatuck Tournaments — allow negative team handicaps.
--
-- Plus handicaps (negative numbers, written "+3" for a -3 stored value)
-- are real in Calcutta-style golf events: a scratch-or-better player
-- gives strokes back to the field. Migrations 0054 and 0055 capped the
-- column and both RPCs at 0–200. Widen the floor to -50, leave the
-- ceiling at 200.
...
alter table public.teams
  ...
  check (handicap is null or handicap between -50 and 200);
...
begin
  ...
  if p_handicap is not null and (p_handicap < -50 or p_handicap > 200) then
    raise exception 'handicap must be between -50 and 200';
  end if;
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_handicaps) loop
    ...
    if v_hcp is not null and (v_hcp < -50 or v_hcp > 200) then
      raise exception 'handicap must be between -50 and 200';
    end if;
